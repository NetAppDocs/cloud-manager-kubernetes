...
* link:task-creating-connectors-azure.html[Create a Connector from Cloud Manager] (recommended)
* link:task-launching-azure-mktp.html[Create a Connector from the Azure Marketplace]
* link:task-installing-linux.html[Install the Connector on an existing Linux host]
