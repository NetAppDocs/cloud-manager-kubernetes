...
* link:task-creating-connectors-aws.html[Create a Connector from Cloud Manager] (recommended)
* link:task-launching-aws-mktp.html[Create a Connector from the AWS Marketplace]
* link:task-installing-linux.html[Install the Connector on an existing Linux host in AWS]
